...
import { Eye, Loader2, Plus, Trash2Icon } from "lucide-react";
...
import PendingTransferShipments from "./PendingTransferShipments";

function TransferTaskCreateNew() {
  ...
  const fetchTransferAreas = useCallback(() => {
  ...
  useEffect(() => {
    ...
    fetchTransferAreas();
  }, [dispatch, trucks, truck_drivers, fetchTransferAreas]);

  const validateForm = () => {
    const newErrors = {
      truck_id: truck ? '' : t('Truck is required'),
      truck_driver_id: truckDriver ? '' : t('Truck Driver is required'),
      destinations: destinationRows.some(row => row.selectedDestination) ? '' : t('At least one destination is required')
    };
    setErrors(newErrors);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const currentTabId = generateTabId("/shipments/new-create-transfer-task");
      ...
      <Card>
        ...
        <form onSubmit={handleSubmit}>
            ...
            <div className="grid sm:grid-cols-1 lg:grid-cols-2 gap-4 mt-2">
              ...
              <Card className="">
                ...
                <CardContent>
                  ...
                  {errors.destinations && (
                    <p className="mt-1 text-sm text-red-500">{errors.destinations}</p>
                  )}
                </CardContent>
              </Card>
            </div>
          ...
          <CardFooter className="flex flex-col md:flex-row md:justify-between gap-4">
            ...
            <Button variant="secondary" type="button" className="mt-4 ml-2" onClick={() => setShowDetails(!showDetails)}>
              {showDetails ? t("Hide") : t("Show")} {t("Details")} <Eye />
            </Button>
          </CardFooter>
        </form>
      </Card>
      {showDetails && (
        <PendingTransferShipments />
      )}
    </div>
  );
}

export default TransferTaskCreateNew;