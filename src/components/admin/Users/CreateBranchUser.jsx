...
function BranchUserCreate() {
  ...
  const validateForm = (formData) => {
    const newErrors = {
      ...
      branch_id: formData.get("branch_id") ? "" : t("Branch is required"),
  ...
  const handleSubmit = async (e) => {
    ...
    const branchIds = formData.getAll("branch_id");
    setIsLoading(true);
    try {
      const payload = {
      ...
      const response = await axiosMerchant.post(
        `users/store_branch_admin/`,
        payload
      );
      toast.success(response.data.message);
      // Navigate to users list and pass tab ID to close
      const currentTabId = generateTabId("/create-branch-admin");
  ...
  return (
    <div>
      <Card className="">
        <CardHeader>
          <CardTitle>{t("Create Branch Admin")}</CardTitle>
        </CardHeader>
