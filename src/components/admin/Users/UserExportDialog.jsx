...
function UserExportDialog({ open, onClose, onSubmitSuccess, users }) {
    ...
    const handleExport = async () => {
        setIsLoading(true);
        try {
            const response = await axiosMerchant.get('users/export', {
                params: {
                ...
                <div className="grid gap-4 py-4">
                    <div className="flex items-center space-x-2">
                        ...
                        <Label htmlFor="include-all" className="cursor-pointer">
                            {t("Include all users (not just current page)")}
                        </Label>
                    </div>

                    <div className="text-sm text-muted-foreground">
                        {includeAll
                            ? t("Exporting all users from the system")
                            : t("Exporting {count} users from current page", { count: users?.length })}
                    </div>
                </div>
