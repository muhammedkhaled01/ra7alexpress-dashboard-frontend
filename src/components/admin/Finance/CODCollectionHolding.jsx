...
function CODCollectionHolding() {
  ...
  const total = Number(runsheet.total_amount || 0);
  const paid = Number(runsheet.paid_amount || 0);
  const remaining = Math.max(0, +(total - paid).toFixed(3));

  const [cash, setCash] = useState(0);
  const [bank, setBank] = useState(0);

  const sum = +(Number(cash || 0) + Number(bank || 0)).toFixed(3);
  const exactMatch = sum === remaining; // أو = total لو ما فيش paid

  const overPay = sum > remaining;
  const invalid = cash < 0 || bank < 0 || overPay;

  const fetchDrivers = useCallback(async () => {
    try {
      const response = await axiosMerchant.get(
        "cod_collection/drivers/holding"
      );
  ...
  const fetchCOD = useCallback(async () => {
    ...
    try {
      const params = {
        ...
        create_date_start:
          `${createTimeRange.from} ${createTimeRange.from_time}` || "",
        create_date_end:
          `${createTimeRange.to} ${createTimeRange.to_time}` || "",
        confirm_date_start:
          `${confirmTimeRange.from} ${confirmTimeRange.from_time}` || "",
        confirm_date_end:
          `${confirmTimeRange.to} ${confirmTimeRange.to_time}` || "",
      ...
      {displayMode === "chart" ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mt-4">
          ...
          <Card className="p-4 flex flex-col items-center">
            ...
            <p className="text-2xl font-bold">
              {formatDecimalValue(
                performanceSummary?.total_money,
                decimalPrecision
              ) || "0.00"}
            </p>
          </Card>
          <Card className="p-4 flex flex-col items-center">
            ...
            <p className="text-2xl font-bold">
              {formatDecimalValue(
                performanceSummary?.signed_money?.total,
                decimalPrecision
              ) || "0.00"}
            </p>
          </Card>
          <Card className="p-4 flex flex-col items-center">
            ...
            <p className="text-2xl font-bold">
              {formatDecimalValue(
                performanceSummary?.signed_money?.cash,
                decimalPrecision
              ) || "0.00"}
            </p>
          </Card>
          <Card className="p-4 flex flex-col items-center">
            ...
            <p className="text-2xl font-bold">
              {formatDecimalValue(
                performanceSummary?.signed_money?.pos,
                decimalPrecision
              ) || "0.00"}
            </p>
          </Card>
...
export default CODCollectionHolding;
