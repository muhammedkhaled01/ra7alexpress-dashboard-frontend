...
const MerchantShipmentCreate = forwardRef(
    ({bulkMode = false, onShipmentSaved} = {}, ref) => {
        ...
        useEffect(() => {
            if (locationState.state?.shipment) {
                const shipment = locationState.state.shipment;
                console.log(shipment, 'shipment')
                setProof(shipment?.proofs?.[0])
                console.log(proof, 'proof')
                setName(shipment.recipient?.name || "");
                setTrackingNo(shipment?.tracking_no)
                setEmail(shipment.recipient?.email || "");
                setPhone(shipment.recipient?.cellphone || "");
                setAlternatePhone(shipment.recipient?.alternate_phone || "");
                setStreetAddress(shipment.recipient?.street_address || "");
                setZipcode(shipment.recipient?.zipcode || "");

                // Set shipment details
                setPaymentType("");
                setAmount(shipment.amount || "");
                setCommission(shipment.delivery_fee || "");

                // Set shipper and merchant if available
                if (shipment.shipper_id) {
                    const shipper = shippers?.find(s => s.id === shipment.shipper_id);
                    if (shipper) {
                        setShipperValue({
                            value: shipper.id,
                            label: shipper.name
                ...
                setMerchant(shipment.merchant_id);
                setFeePayer({value: shipment.fee_payer || "customer"});
                // Clear the location state to prevent re-population on re-renders
                window.history.replaceState({}, document.title);
            }
        }, [locationState.state, shippers, merchants]);
        ...
        const handleSubmit = async (e) => {
            ...
            try {
                ...
                form.append("merchant_id", merchant);

                form.append("tracking_no", trackingNo);
                ...
                const response = await axiosMerchant.post(`merchant/shipments/registerShipment`, form);
        ...
        const setStateValue = async (value) => {
            ...
            await axiosMerchant
                .get(`merchant_commissions/by_state/${merchant}/${value.value}`)
                .then((res) => {
                                    ...
                                    <Card>
                                        <RecipientForm
                                            ...
                                            onStateChange={(opt) => {
                                                axiosMerchant
                                                    .get(
                                                        `merchant_commissions/by_state/${merchant}/${opt.value}`
                                                    )
                                    ...
                                    <Card>
                                        ...
                                        <CardContent>
                                            ...
                                            <div className="flex items-center gap-4 mt-2">
                                                <div
                                                    className="bg-blue-50 dark:bg-gray-800 rounded px-3 py-2 text-xs space-x-4">
