...
export default function CreateScheduledAction({ onClose, onSubmit }) {
    ...
    const [form, setForm] = useState({
        ...
        action_type: actionOptions[0].value,
        status: 'active'
    });
    ...
    const handleSubmit = async (e) => {
        ...
        const apiPayload = {
            ...
            action_display: selectedActionObject ? t(selectedActionObject.label) : '',
            action_payload: null,
            status: form.status.toLowerCase(),
        };
        if (!apiPayload.cron_expression || !apiPayload.schedule_display) {
            setErrors(prev => ({
                ...prev,
                schedule_display: prev.schedule_display || [t('Schedule configuration is incomplete. Cron expression could not be generated.')]
            }));
            toast.error(t('Schedule configuration is incomplete.'));
            setIsLoading(false);
            return;
        }
        try {
            const responseData = await onSubmit(apiPayload);
            if (responseData && responseData.success && responseData.message) {
                toast.success(responseData.message);
            } else if (responseData && responseData.success) {
                toast.success(t('Scheduled action created successfully!'));
            } else if (!responseData || !responseData.success) {
                toast.error(responseData?.message || t('An issue occurred, but no specific error message was provided.'));
            }
            onClose();
        } catch (error) {
            console.error('Error creating scheduled action (caught in child):', error);
            if (error.response && error.response.data) {
                if (error.response.status === 422 && error.response.data.errors) {
                    setErrors(error.response.data.errors);
                    const mainErrorMessage = error.response.data.message || t('Please correct the errors in the form.');
                ...
                toast.error(t('Failed to connect to the server or an unknown error occurred.'));
            }
        } finally {
            setIsLoading(false);
        }
    ...
    return (
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            <div className="input-container">
                <Label htmlFor="action_name">{t('Action Name')} <RequiredField /></Label>
                <Input
                    id="action_name"
                    name="action_name"
            ...
            <div className="input-container">
                <Label htmlFor="frequency">{t('Frequency')} <RequiredField /></Label>
                <Select
                    id="frequency"
                    name="frequency"
                    options={frequencyOptions.map(opt => ({ ...opt, label: t(opt.label) }))}
            ...
            {form.frequency !== 'CUSTOM' && (
                <>
                    {form.frequency === 'WEEKLY' && (
                        <div className="input-container">
                            <Label htmlFor="day_of_week">{t('Day of Week')} <RequiredField /></Label>
                            <Select
                                id="day_of_week"
                                name="day_of_week"
                                options={daysOfWeek.map(opt => ({ ...opt, label: t(opt.label) }))}
                    ...
                    <div className="input-container">
                        <Label htmlFor="time">{t('Time')} <RequiredField /></Label>
                        <Input
                            id="time"
                            name="time"
                            type="time"
            ...
            {form.frequency === 'CUSTOM' && (
                <div className="input-container">
                    <Label htmlFor="cron_expression_input">{t('Cron Expression')} <RequiredField /></Label>
                    <Input
            ...
            <div className="input-container">
                <Label htmlFor="action_type">{t('Action')} <RequiredField /></Label>
                <Select
                    id="action_type"
                    name="action_type"
                    options={actionOptions.map(opt => ({ ...opt, label: t(opt.label) }))}
                    value={actionOptions.find(option => option.value === form.action_type) ? { value: form.action_type, label: t(actionOptions.find(option => option.value === form.action_type).label) } : null}
...
CreateScheduledAction.propTypes = {
