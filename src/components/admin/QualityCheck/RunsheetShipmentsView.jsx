            ...
            <table className="min-w-full  ">
              ...
              <tbody>
                {record?.map((shipment, index) => {
                  return <tr key={index}>
                    <td className="px-4 py-2 border-b">{shipment.shipment?.tracking_no || shipment.tracking_no}</td>
                    <td className="px-4 py-2 border-b">{shipment.shipment?.consignee || shipment.shipment_details?.consignee?.name}</td>
                    <td className="px-4 py-2 border-b">{consigneeAddress(shipment.shipment?.consignee || shipment.shipment_details?.consignee)}</td>
                    <td className="px-4 py-2 border-b">{shipment.shipment?.payment_type || shipment.shipment_details?.payment_type}</td>
                    <td className="px-4 py-2 border-b">{shipment.shipment?.amount || shipment.shipment_details?.amount}</td>
                  </tr>
                })}
              </tbody>
            </table>
