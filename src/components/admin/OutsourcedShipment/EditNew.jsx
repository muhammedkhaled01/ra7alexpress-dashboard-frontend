...
function TaskEditNew({ shipment: _shipment, isOpen, setIsOpen, onClose, onSubmitSuccess }) {
  ...
  useEffect(() => {
    ...
    if (isOpen && _shipment) {
      ...
      setGovernorateValue({
        value: _shipment?.consignee?.governorate?.id,
        label: `${_shipment?.consignee?.governorate?.en_name} - ${_shipment?.consignee?.governorate?.ar_name}`
      });
      setStateValue({
      ...
      setLocation(_shipment?.consignee?.location || "");
  ...
  useEffect(() => {
    if (countryValue) {
      setFilteredGovernorates(
        governorates?.filter((governorate) => governorate.country_id === countryValue.value)
      );
      setFilteredStates(
        states?.filter((state) => state.governorate_id === governorateValue.value)
      );
      if (stateValue) {
        setFilteredCities(
  ...
  }, [countryValue, states, governorates, governorateValue]);
