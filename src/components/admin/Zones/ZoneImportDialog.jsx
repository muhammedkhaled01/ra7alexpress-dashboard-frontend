...
function ZoneImportDialog({ open, onClose, onSubmitSuccess }) {
    ...
    const handleImport = async () => {
        ...
        try {
            ...
            const response = await axiosMerchant.post('zones/import', formData, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                },
            });
            if (response.data.success === true) {
                toast.success(response.data.message || t("Zones imported successfully"));
                onClose();
                if (onSubmitSuccess) {
                    onSubmitSuccess();
                }
    ...
    const resetForm = () => {
        setFile(null);
        if (document.getElementById('zone-import-file')) {
            document.getElementById('zone-import-file').value = '';
        }
    };
    ...
    const downloadTemplate = () => {
        const templateData = [
            ['Name', 'Coordinates', 'Governorate IDs', 'State IDs', 'Place IDs', 'Owner Type', 'Owner ID', 'Created At'],
            ['Zone A', '{"type":"Polygon","coordinates":[[[55.1,25.1],[55.2,25.1],[55.2,25.2],[55.1,25.2],[55.1,25.1]]]}', '1,2', '5,6', '10,11', 'Hub', '1', '2024-01-01 10:00:00'],
            ['Zone B', '{"type":"Polygon","coordinates":[[[55.3,25.3],[55.4,25.3],[55.4,25.4],[55.3,25.4],[55.3,25.3]]]}', '3', '7,8', '12', 'Branch', '2', '2024-01-02 11:00:00']
        ];
    
        // تحويل إلى CSV مع معالجة الاقتباسات بشكل صحيح
        ...
        // إنشاء ملف للتحميل مع BOM لضمان دعم UTF-8
        const bom = '\uFEFF';
        const blob = new Blob([bom + csvContent], { type: 'text/csv;charset=utf-8;' });
        const url = URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.setAttribute('href', url);
        link.setAttribute('download', 'zones_import_template.csv');
    ...
    return (
        <Dialog open={open} onOpenChange={(isOpen) => !isOpen && handleClose()}>
            <DialogContent className="sm:max-w-2xl bg-background text-foreground">
                <DialogHeader>
                    <DialogTitle className="text-foreground">{t("Import Zones")}</DialogTitle>
                </DialogHeader>

                <div className="grid gap-4 py-4">
                    <div className="grid gap-2">
                        <Label htmlFor="zone-import-file" className="text-foreground">
                            {t("Select Excel File")}
                        </Label>
                        <Input
                            id="zone-import-file"
                    ...
                    <div className="p-4 bg-blue-100 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                        ...
                        <ul className="text-sm text-blue-700 dark:text-blue-300 space-y-1">
                            <li>• <strong>{t("Required columns")}:</strong> name, coordinates</li>
                            <li>• <strong>{t("Coordinates format")}:</strong> {t("Valid GeoJSON Polygon string")}</li>
                            <li>• <strong>{t("Example coordinates")}:</strong> {"{'type':'Polygon','coordinates':[[[55.1,25.1],[55.2,25.1],[55.2,25.2],[55.1,25.2],[55.1,25.1]]]}"}</li>
                    ...
                    <div className="p-4 bg-amber-100 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg">
                        ...
                        <ul className="text-sm text-amber-700 dark:text-amber-300 space-y-1">
                            <li>• {t("Zone names must be unique")}</li>
                            <li>• {t("Coordinates must be valid GeoJSON Polygon format")}</li>
                            <li>• {t("Polygons must be closed (first and last points must be the same)")}</li>
                            <li>• {t("Coordinates must be in valid longitude/latitude format")}</li>
