...
function MerchantBranchEdit() {
  ...
  const accessAbility = can("Merchant Branch update") || hasRole("Merchant");
  ...
  const fetchBranchData = async () => {
    setLoading(true);
    try {
      const response = await axiosMerchant.get(`merchant/branches/edit-branch/${id}`);
      ...
      // Wait for data to load before setting selects
      setTimeout(() => {
        setSelectValues(branch);
      }, 500);
  ...
  const setSelectValues = (branch) => {
    // Set country
    if (countries && branch.country) {
      const country = { value: branch.country.id, label: branch.country.name };
      _setCountryValue(country);
      setCountryValue(country);
    }

    // Set status
    const status = { value: branch.status, label: t(branch.status === 'active' ? 'Active' : 'Inactive') };
    setStatusValue(status);

    // Set other location values after country is set
    setTimeout(() => {
      if (branch.governorate) {
        const governorate = {
          value: branch.governorate.id,
          label: `${branch.governorate.en_name} / ${branch.governorate.ar_name || ''}`
        };
        _setGovernorateValue(governorate);
        setGovernorateValue(governorate);
      }

      if (branch.state) {
        const state = {
          value: branch.state.id,
          label: branch.country?.name === "Egypt"
            ? `${branch.state.en_name} / ${branch.state.ar_name || ''}`
            : branch.state.en_name
        };
      ...
      if (branch.place) {
        const place = {
          value: branch.place.id,
          label: `${branch.place.en_name} / ${branch.place.ar_name || ''}`
        };
        setPlaceValue(place);
      }

      if (branch.city) {
        const city = { value: branch.city.id, label: branch.city.name };
        setCityValue(city);
      }
    }, 200);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axiosMerchant.post(`merchant/branches/${id}/update`, form);
            ...
            <CardContent>
              <div className="grid sm:grid-cols-1 lg:grid-cols-2 gap-4 mt-4">
                <div className="input-container">
                  <label htmlFor="name">{t("Branch Name")} *</label>
                  <Input
                    id="name"
                    name="name"
                    type="text"
...
export default MerchantBranchEdit;
